...
while ( $count <= $#ARGV ) {
  ...
  for ($a) {
    ...
    /^-?b$/
      && do { @dirs = ( @dirs, $s, $r ); $count++; next; }; # both: roiling goes first due to more debug text
...
if ( $#dirs == -1 ) {
  if ( -f "story.ni" && ( getcwd() =~ /games.inform/ ) ) {
    ...
    if ( ( lc($shortDir) eq "roiling" ) || ( lc($shortDir) eq "shuffling" ) ) {
      @dirs = ($shortDir);
      print
"Running only $shortDir. Leave this directory or use the -2 flag to force default (both dir) behavior.\n";
    }
  }
  else {
    @dirs = ( $r, $s );
...
sub alphSource {
  ...
  while ( $a = <A> ) {
    if ( ( $a =~ /^blurb/ )
      && ( $a !~ /\[noalf\]/ )
      && ( $a !~ /^blurb\t\"/ )
      && ( $b !~ /magnifs/ ) )    #blurb then quote appears in SA
    {
      $count++;
  ...
  if ($dupes) {
    print "$dupes duplicate(s) found. Results below.\n$dupeString";
  }
  else { print "No duplicates found. Hooray!\n"; }
  ...
  if ( $nuDel != $niSize ) {
    die
"Something went wrong. File sizes aren't equal! New=$nuSize + $deletedBytes = $nuDel != Old=$niSize. Maybe check CR's.";
  }
...
"c:\\Program Files (x86)\\Inform 7\\Inform7\\Extensions\\Andrew Schultz\\temp.i7x";
  ...
  if ( !$dontcopy ) {
    ...
    $thisOutString =
      "$modFileShort," . ( -s "$modFile" ) . ",$dupes,$dateForm\n";
    ...
    if ( !$seriouslyTesting ) {
      `$cmd`;
      print "Copying over $outFileName $fileName.\n$cmd\n";
    }
...
sub sortTheTable {
  ...
  while ( $a = <A> ) {
    ...
    if ( ( $#c == 1 ) || ( $#c > 2 ) ) {
      $totBad++;
      $badError .= ("Uh-oh, wrong # of quotes at $short line $.\n$#c:$a");
    }
    if ( $a =~ /^'/ ) {
      $totBad++;
      $badError .=
        ("Uh-oh, single quote line start at $short line $., bailing.");
    }
    if ( $a !~ /^\"/ ) {
      if ( $a =~ /^[a-z]/i ) {
        $totBad++;
        $badError .= (
"Uh-oh, you started a table line with a character: $short # $., bailing."
        );
      }
      if ( $a =~ /^$ch/ ) {
        $totBad++;
  ...
  if ($badError) {
    print "TEST RESULTS:$short tables bad fail,1,0,0,0,$badError\n";
    die;
  }
  if ($verbose) {
    printf(
      "%2d: Sorting $thisTable, " . ( $#ary + 1 ) . " total elements.\n",
      $count
    );
  }
  if ( $#ary < 23 ) {
  ...
  for ( 0 .. $#ary2 ) {
    ...
    $temp2 =~ s/'//g;
    if ( $isDone{$temp2} ) {
      if ( $thisTable ne $lastDupTable ) {
        $dupeString .= "****TABLE $thisTable\n";
      ...
      if ( $lines2 - $idt == 1 ) {
        $addDupe =
"$temp ($lines2-$short) is an immediate duplicate($table{$temp2}). Weeding out.\n";
        $deletedBytes += length( $ary2[$_] );
        $ary2[$_] = "";
      }
      else {
        $addDupe =
"$temp ($lines2-$short) is duplicated from line $isDone{$temp2} $table{$temp2}.\n";
      }
      $dupeString .= $addDupe;
    }
    elsif ( $_ > 0 ) {
      if ( ( $ary2[ $_ - 1 ] ) && ( $ary2[$_] =~ /\Q$ary2[$_-1]/i ) ) {
        ...
        print
"LastLineCheck: $temp ($lines2-$short) is duplicated from line $isDone{$temp2}.\n";
        $dupes++;
        $dupeString .= "($_) $thisTable ($lines2 from previous line)\n";
      }
    }
