...
for my $thisarg ( 0 .. $#ARGV ) {
  ...
  if ( $thisMatch =~ /\./ ) {
    print "$thisMatch has periods, removing.\n";
    $thisMatch =~ s/\.//g;
  }
...
sub addPrev {
  my $newAdd = lc( $_[0] );
  $newAdd =~ s/[0-9=]//g;
  if ( $inPrev{$newAdd} ) { print "$newAdd already in history.\n"; }
  else {
...
OUTER:
  while ( $a = <A> ) {
    ...
    my $toAdd = "";
    for my $q ( keys %times ) {
      if ( defined( $mytimes{$q} ) && ( $mytimes{$q} > 0 ) ) {
        $toAdd .= uc($q) x $mytimes{$q};
      }
    }

    if ( !$eqOnly ) {
      if ( !$toAdd ) { print "!"; $matches .= lf($a); }
      print "/" . alfo($toAdd);
    }
    else {
      if ( $matchFlag && !$toAdd ) { $matches .= lf($a); }
    }
    if ( $linked{ alfo($a) } ) {
      if ( ($mustFirstLast) && ( $nameType{ alfo($a) } eq "L" ) ) { next; }
      print "=$nameType{alfo($a)}|$linked{alfo($a)}/$bkwd{alfo($a)}";
      if ($eqOnly) {
        $count++;
        if ( $count == 5 ) { print "\n"; $count = 0; }
        else               { print ", "; }
      ...
      $bkwd{ alfo($toAdd) }     = $a;
      $nameType{ alfo($toAdd) } = uc( $_[0] );
      $nameType{ alfo($toAdd) } =~ s/^(.).*/$1/g;
    }
    if ( !$eqOnly ) {
      if ( $count == 5 ) { print "\n"; $count = 0; }
      else               { print ", "; }
