...
my @toAlt = ("a-b", "d-t", "i-n", "m-e", "mle", "n-t", "sim", "ta", "toti", "tt");
...
sub readInFile
{
my $inFile = "C:/Program Files (x86)/Inform 7/Inform7/Extensions/Andrew Schultz/$_[0] Random Text.i7x";
my $notTheFirst = 0;
my $inTable = 0;
...
####NON TEXTY STUFF. CENSOR SWEARS
	...
	if ($l2 =~ /\[/) { print "TITLE WITH BRACKETS: $l2\n"; $failures++; push(@failLines, $line); }
	else
	{
...
sub readBeforeAfter()
{
  ...
  while ($line = <A>)
  {
    if ($line =~ /=====/) { $idx++; } else { $genStr[$idx] = $genStr[$idx] . $line; }
  }
  close(A);
}
